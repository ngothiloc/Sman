...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['add_news_bao_trongnuoc'])) {
        $title = $_POST['title'];
        ...
        // Kiểm tra kích thước ảnh (nếu cần)
        ...
        if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
            ...
            $sql = "INSERT INTO news_bao_trongnuoc (title, content, link, image, post_date) 
                    VALUES (?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssss", $title, $content, $link, basename($_FILES["image"]["name"]), $post_date);
            if ($stmt->execute()) {
                echo "<script>alert('Tin tức đã được thêm thành công!');</script>";
            } else {
                echo "<script>alert('Lỗi khi chèn vào cơ sở dữ liệu: " . $stmt->error . "');</script>";
            }
            $stmt->close();
        } else {
            echo "<script>alert('Lỗi khi tải lên ảnh. Hãy kiểm tra quyền ghi của thư mục \"../uploads/\".');</script>";
        }
    } elseif (isset($_POST['delete_news_bao_trongnuoc'])) {
        $news_id = intval($_POST['news_id']);
        $delete_sql = "DELETE FROM news_bao_trongnuoc WHERE id = ?";
...
$sql = "SELECT * FROM news_bao_trongnuoc ORDER BY id DESC";
...
<body>
    ...
    <aside id="sidebar" class="sidebar">
        ...
        <ul class="sidebar-nav" id="sidebar-nav">
            ...
            <li class="nav-item">
                <a class="nav-link" data-bs-target="#dangtin-nav" data-bs-toggle="collapse" href="#">
                    <i class="fa-solid fa-microchip"></i><span>Đăng tin</span><i class="bi bi-chevron-down ms-auto"></i>
                </a>
                <ul id="dangtin-nav" class="nav-content collapse show" data-bs-parent="#sidebar-nav">
                    <li>
            ...
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form action="dangtin_bao_trongnuoc.php" method="post" enctype="multipart/form-data">
                                <div class="mb-3">
                                ...
                                <button type="submit" name="add_news_bao_trongnuoc" class="btn btn-primary">Thêm tin tức</button>
                            </form>
                        </div>
                    </div><!-- End Form Card -->
                                ...
                                <tbody>
                                    ...
                                    if ($result && $result->num_rows > 0) {
                                        while ($row = $result->fetch_assoc()) {
                                            ...
                                            echo '<td>
                                                <form method="post" action="dangtin_bao_trongnuoc.php" style="display:inline;" onsubmit="return confirmDelete();">
                                                    <input type="hidden" name="news_id" value="' . $row["id"] . '">
                                                    <button type="submit" name="delete_news_bao_trongnuoc" class="btn btn-danger btn-sm">Xóa</button>
                                                </form>
                                            </td>';
